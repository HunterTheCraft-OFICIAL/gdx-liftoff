...
import gdx.liftoff.data.libraries.official.GdxAI
import gdx.liftoff.data.libraries.official.TweenEngine
...
enum class Preset {
   ...
   * Includes all official GDX extensions now: Box2D, Box2D-Lights, Freetype, GdxAI, TweenEngine.
  ...
  ANDROID_DEV {
    ...
    override val languages: List<Language> = emptyList()
    override val officialExtensions: Optional<List<Library>>
      get() = Optional.of(listOf(Box2D(), Box2DLights(), Freetype(), GdxAI(), TweenEngine()))
